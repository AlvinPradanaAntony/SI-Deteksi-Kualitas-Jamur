function seg = region_seg(I,init_mask,max_its,alpha,display)
  ...
  %-- nilai default untuk parameter alpha adalah .1
  if(~exist('alpha','var')) 
    alpha = .2; 
  end
  %-- default behavior is to display intermediate outputs
%   menampilkan output perantara
  if(~exist('display','var'))
    display = true;
  end
  %-- ensures image is 2D double matrix
  %-- Memastikan gambar 2D
  I = im2graydouble(I);    
  
  %-- Create a signed distance map (SDF) from mask
  %-- Membuat Peta Jarak dari mask
  ...
  for its = 1:max_its   % Note: no automatic convergence test
    ...
    dphidt = F./max(abs(F)) + alpha*curvature;  % penurunan gradien untuk meminimalkan energi
